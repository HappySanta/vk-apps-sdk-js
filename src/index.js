...
export const VK_API_INTERNAL_ERROR = 10;
...
export const SOFT_ERROR_CODES = [
  ...
  VK_API_INTERNAL_ERROR, //Неизвестная ошибка, но можно повторить еще раз
];
...
export default class VkSdk {
   ...
   */
  static async api(method, params = {}, scope = '', retry = 7) {
    const backoff = new ExponentialBackoff(1000, retry);
    let lastError = new VkSdkError('Unknown api error');
    ...
    while (backoff.canRetry()) {
      ...
      if (!VkSdk.tokenCache[scope] && !p.access_token) {
        ...
        VkSdk.tokenCache[scope] = await getTokenWithScope(scope);
        VkSdk.log('[' + method + '] Token fetched: ' + (VkSdk.tokenCache[scope] || '').substr(0, 5) + '...');
      }
      ...
      try {
        return await VkSdk.callAPIMethod(method, p, true);
      } catch (e) {
        lastError = e;

        const isApiErr = (e.type === VkSdkError.API_ERROR);
        const isNetworkErr = (e.type === VkSdkError.NETWORK_ERROR);

        if (isApiErr && (Number(e.code) === VK_API_AUTH_FAIL)) {
          if (passedTokenInParams) {
            throw e;
          } else {
            delete VkSdk.tokenCache[scope];
            VkSdk.log('[' + method + '] retry api call by VK_API_AUTH_FAIL: ' + e.message);
            await backoff.wait();
            continue;
          }
        }

        if (isNetworkErr || (isApiErr && (SOFT_ERROR_CODES.indexOf(e.code) !== -1))) {
          VkSdk.log('[' + method + '] retry api call : #' + e.message);
          await backoff.wait();
        } else {
          throw e;
        }
  ...
  return await getTokenQueue.call(async () => {
    ...
    while (backoff.canRetry()) {
      try {
        ...
        VkSdk.log('getTokenWithScope fail, retry: ' + backoff.attempt + ' msg: ' + e.message);
        await backoff.wait();
      }
    }
    throw lastError;
